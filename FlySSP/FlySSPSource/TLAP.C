/*------------------------------------------------------------------------
  TLAP.C
      ПРЕОБРАЗОВАНИЕ ПОЛИНОМА ЛАГЕРРА В АЛГЕБРАИЧЕСКИЙ ПОЛИНОМ
...
void tlap(double a, double b, double pol[], int n, double c[], double work[])
{
int    j, k;
double fi, h, p, q, q1, q2;
...
for( j = 3; j <= n; j++ ){
	fi = fi + 1.e0;	q = 1.e0/fi;
	q1 = q - 1.e0; 	q2 = 1.e0 - q1 - b*q;
	q = q*a;       	p = 0.e0;
	for( k = 2 ; k <= j; k++ ){
		h = -p*q + work[2*k - 2-1]*q2 + work[2*k - 3-1]*q1;
		p = work[2*k - 2-1];
		work[2*k - 2-1] = h;
		work[2*k - 3-1] = p;

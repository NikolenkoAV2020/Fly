//-------------------------------------------------------------------------------
// TaskTurnsPredictionHAvr.cpp : 
// А. Николенко 08.08.2018
//-------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
int RunUpSysEstimateNU(ZLSF& LSF, ZNU& NU0, ZNU& NUK, double uz, double e1z, 
					   double e2z, double az, double havrz=0) ; 
...
int TaskTurnsPredictionHAvr()
{
	...
	ZModExternHAvr KA ;			// Модель движения КА
	...
	// Выбор параметров прогнозируемой орбиты - солнечно синхронная орбита
	{
		...
		rc = MBuildSSOMIV_hs(Hs, T, i, p, e1, e2, nullptr); if (rc) return rc;
		...
		ZNU NUK ;
		rc = RunUpSysEstimateNU(LSF, NU, NUK, 0, e1, e2, 0, Hs); if (rc) return rc;
		NU = NUK ;
	}
	...
	cout << "\r       turn  " << Vitn << "...    ";
	rc = KA.GoToVit(Vitn, &SI); if (rc) return rc;
	Vit = Vitn ;

...
//
// 24-й вариант LNS-маневра, но с коррекцией аргумента широты КА u на заданный
// момент времени t, вместо коррекции долготы L.
//
// u,T+h,Bh или u,T+e,w для ОМИВ, ДУ, ЭО
//-----------------------------------------------------------------------------
#include "stdafx.h"
...
int TaskMagicFly241(int TypeOrb)
{
	...
	//-------------------------------------------------------------------------
	MS->MZ.ORB() = TypeOrb; // = SPO_ORB_OMIV;
	...
	if (varNU == 1) {
		...
		MS->MZ.PART() = 5345.66;	// Драконический период
		...
		MS->MZ.PARw() = 180;		//	
		MS->MZ.PARu() = 102.0;		// 
		MS->MZ.PARt() <<DATETIMEDMB(14, 12, 2015, 18, 20, 0, 0);
	}
	else {
		// ТЕСТ - 2 (сравнение с BVN-маневром)
		MS->MZ.VITIMP(0) = 4;		// Виток первого импульса

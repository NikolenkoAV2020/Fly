/*------------------------------------------------------------------------
  TCNP.C
      ПРЕОБРАЗОВАНИЕ ПОЛИНОМА ЧЕБЫШЕВА В АЛГЕБРАИЧЕСКИЙ ПОЛИНОМ
...
void tcnp(double a,double b,double pol[],int n,double c[],double work[])
{
int    j, k;
double h, p, x0, xd;
...
if( (n - 2) <= 0) goto L_2;
/*        INITIALIZATION */
work[0] = 1.e0; work[1] = b;
work[2] = 0.e0; work[3] = a;
xd = a + a;     x0 = b + b;
